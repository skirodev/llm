...
impl KnownModel for ClipVision {
        ...
        Self: Sized,
    {
        ...
        for i in 0..hyperparameters.t_n_layer {
            let layer = Layer {
                // vision
                q_w: tl.load(&format!(
    ...
    fn evaluate(
        ...
        static IMAGE_BATCH_SIZE: usize = 2;
        ...
        let outputs = session.compute(self.context.clone(), flattened_imgs, |builder| {
            let imgs: Vec<Vec<u32>> = flattened_imgs.chunks(1 * 3 * 224 * 224)
                .map(|chunk| chunk.to_vec())
                .collect();
            ...
            //assert_eq!(self.vision_patch_embeddings.get_ne()[0], 256);
            assert_eq!(self.vision_patch_embeddings.get_ne()[1], 2304);
            ...
            assert_eq!(inp.get_ne()[0], 14);
            assert_eq!(inp.get_ne()[1], 14);
            ...
            for il in 0..v_n_layer {
                ...
                // layernorm1
                {
                    ...
                    current = ctx0.op_add(
                        &ctx0.op_mul(&ctx0.op_repeat(&self.layers[il].ln_1_w, &current), &current),
        ...
        pub fn extract_embeddings(
            ...
            if let Some(embeddings) = &mut output_request.embeddings {
                assert_eq!(n_embd, 512);
                assert_eq!(n, IMAGE_BATCH_SIZE);
                embeddings.resize(n_embd * IMAGE_BATCH_SIZE, 9.0);
                ...
                embeddings.copy_from_slice(&all_embeddings[n_embd * 0..n_embd * IMAGE_BATCH_SIZE]);
